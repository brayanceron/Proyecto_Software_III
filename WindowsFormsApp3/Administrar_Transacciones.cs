...
namespace WindowsFormsApp3
{
    ...
    {
        public Administrar_Transaciones(int boton, string user)
        {
            InitializeComponent();
            if (user.Equals("AUXILIAR"))
            {
                txb_estadopres.SelectedIndex=0;
                txb_estadopres.Enabled = false;
                txb_estadoinver.SelectedIndex = 0;
                txb_estadoinver.Enabled = false;
            }
            if (user.Equals("Jefe"))
            {
        ...
        private void chbx_gar_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (chbx_gar.Checked == true)
            {
                ...
                Administrar_Garantia nuevo = new Administrar_Garantia(Convert.ToInt32(codigogarantia),null,null);
        ...
        private void btn_registrarprestamo_Click(object sender, EventArgs e) {
            ...
            ///VERIFICANDO SOPORTE DE LA DEUDA
            if (chbx_fiador.Checked == true)
            {
                sql += "'" + fiadorcad + "',null)";
                if (metodos.buscar("fiador", "cedulafiador", fiadorcad) == false) { MessageBox.Show("El fiador no ha sido encontrado"); return; }
